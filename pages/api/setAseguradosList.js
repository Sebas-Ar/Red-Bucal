...
const handler = async (req, res) => {
    if (req.method === 'POST') {
        ...
        const entity = await mongoClient.db.collection('bussines').findOne({ RUC })
        ...
        for (let i = 9; i < data.length; i++) {
            ...
            if (!data[i][3]) {
                erroMessage[numErrors].errorDate = 'El campo de la fecha de nacimiento se encuentra vacio'
            }
        ...
        entity.identifications.forEach(async (user) => {
            ...
            await mongoClient.db.collection('users').update(
        ...
        console.log('numero de ususarios que se repiten')
        console.log(userContinue.length)
        ...
        for (let i = 9; i < data.length; i++) {
            ...
            if (!userExist) {
                ...
                const userInList = userContinue.find(user => user.id === data[i][1] + '')
                if (!userInList) {
                    userContinue.push({
                        id: data[i][1] + '',
                        name: data[i][0]
        ...
        const business = await mongoClient.db
            .collection('bussines')
            .findOneAndUpdate(
                { RUC },
                {
