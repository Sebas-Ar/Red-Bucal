...
const handler = async (req, res) => {
    ...
    if (req.method === 'GET') {
        console.log('getting insurance list')
        const insuranceList = await mongoClient.db
            .collection('bussines')
            .find(
                {},
                {
            ...
            .toArray()
        console.log('insurance list recieved')
        ...
        await mongoClient.db.collection('bussines').deleteOne({ RUC })

        const userList = await mongoClient.db.collection('users').updateMany(
            { RUC },
            {
                $set: {
                    RUC: '',
