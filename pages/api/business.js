                ...
                if (countEmail) {
                    ...
                    if (countRUC) {
                        ...
                        for (let i = 6; i < data.length; i++) {
                            ...
                            let countId = await req.db
                                .collection("users")
                                .countDocuments({ identification });

                            const userCountEmail = await req.db
                                .collection("users")
                            ...
                            if (userCountEmail) {
                                erroMessage[numErrors][
                                    "errorEmail"
                                ] = `ya existe un usuario registrado con el email: ${userEmail}`;
                            }

                            if (countId) {
                                erroMessage[numErrors][
                                    "errorId"
                                ] = `ya existe un usuario registrado con la cedula: ${identification}`;
                            }
