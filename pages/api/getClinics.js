...
const handler = async (req, res) => {
    ...
    if (all) {
        data = await mongoClient.db.collection('admin').find({}, { projection: { _id: true, name: true, email: true } }).toArray()
    } else if (type && start && end) {
        const startArray = start.split('-')
        const endArray = end.split('-')

        if (name === '') {
            data = await mongoClient.db.collection(type).find(
                {
                    date: {
        ...
        if (name === '') {
            data = await mongoClient.db.collection(type).aggregate([
                { $project: projection }
            ]).toArray()
        } else {
            data = await mongoClient.db.collection(type).aggregate([
