...
import listPercent from '../descuentos/list/listPercent'

const DescuentosList = (props) => {
    const [columns, setColumns] = useState(false)

    useEffect(() => {
        if (listPercent[props.list].list.length > 15) {
            setColumns(true)
        } else {
            setColumns(false)
        }
        ...
        props.active
            ? <div className="content">
                <section>
                    ...
                    <h3>{listPercent[props.list].title}</h3>
                    <ul>
                        {
                            listPercent[props.list].list.map(percent => (
                                <li key={percent}>
                                    {percent}
                                </li>
                            ))
                    ...
                    {
                        listPercent[props.list].exception
                            ? <p>
                                {listPercent[props.list].exception}
                            </p>
                            : ''
